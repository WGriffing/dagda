...
def get_dependencies_from_docker_image(docker_driver, image_name, temp_dir):
    # Init
    filtered_image_name = (
        image_name.replace(" ", "_").replace("/", "_").replace(":", "_")
    )
    # Docker pull for ensuring the 3grander/4depcheck image
    docker_driver.docker_pull("3grander/4depcheck", "0.1.0")
    # Start container
    container_id = docker_driver.create_container(
        "3grander/4depcheck:0.1.0",
        f"python3 /opt/app/4depcheck.py {filtered_image_name} {temp_dir}",
        volumes=[
            tempfile.gettempdir(),
            temp_dir,
            f"{tempfile.gettempdir()}/4depcheck"
            # The previous directory should be resolved as /tmp/4depcheck
        ],
        host_config=docker_driver.get_docker_client().create_host_config(
            binds=[
                f"{tempfile.gettempdir()}:{tempfile.gettempdir()}:rw",
                f"{temp_dir}:{temp_dir}:rw",
                f"{tempfile.gettempdir()}/4depcheck:{tempfile.gettempdir()}/4depcheck:rw",
            ],
        ),
    )
...
def get_filtered_dependencies_info(dependencies, temp_dir):
    ...
    for dependency in dependencies:
        data = (
            dependency["cve_type"]
            + "#"
            + dependency["cve_product"]
            + "#"
            + dependency["cve_product_version"]
            + "#"
            + dependency["cve_product_file_path"].replace(temp_dir, "")
            + "#"
            + dependency["cve_id"]
        )
